...
<head>
    ...
    <title>{{ $metaTitle ?: 'Exxon Blog' }}</title>
    <meta name="author" content="TheCodeholic Blog">
            ...
            <div class="w-full container mx-auto flex flex-col sm:flex-row items-center justify-between text-sm font-bold uppercase mt-0 px-6 py-2">
                ...
                <div class="flex items-center">
                    <form action="{{ route('search') }}" method="get">
                        <input name="q" value="{{ request()->get('q') }}" class="block w-full rounded-md border-0 px-3.5 py-2 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset sm:text-sm sm:leading-6 font-medium" placeholder="Type and hit enter to search anything">
                    </form>
                    @auth
                <div class="flex sm:items-center sm:ml-6">
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="hover:bg-blue-600 hover:text-white rounded flex items-center py-2 px-4 mx-2">
